const YDRZ = (function (win, doc) {
    ...
    console.log('getFingerPrint()：'+opts.optparams.userInformation)
    function getUUID() {
    ...
    win.YDRZ = {
        getSign: function (appId, version) {
            ...
            console.log('options：'+options.userInformation)
            ...
            img.onload = function () {
                ...
                try {
                    request.ajax({
                        request: {url: reqUrl, method: "post", data: JSON.stringify(param)},
                        success: function (res) {
                            if (res.result.body.token != "") {
                                ...
                                console.log('success：'+obj.userInformation)
                                opt.success({
                                    code: obj.code,
                                    message: obj.message,
                                    token: obj.token,
                                ...
                                var obj = {code: res.result.header.resultCode, message: res.result.body.resultDesc};
                                opt.error({code: obj.code, message: obj.message});
                                return obj
                            }
                        },
                        error: function (res) {
                            var obj = {code: "500", message: "接口异常，获取token失败"};
                            opt.error({code: obj.code, message: obj.message});
                            return obj
                        }
                    })
            ...
            img.onerror = function () {
                ...
                opt.error({code: "500", message: "系统异常,图片请求失败"});
                This.isLoaded = true;
                doc.body.removeChild(img)
            }
        }
    ...
    var request = {
        utilCreateXHR: function (options) {
            ...
            var obj = false;
            if (!text) {
                obj = {"code": "ER_NOBODY", "summary": "responseText is empty"}
            }
            ...
            if (!obj) {
                try {
                    ...
                    obj = {"code": "ER_INVALIDJSON", "summary": "responseText is invalid json"}
                }
            }
            return obj
        }, ajax: function (args) {
            ...
            if (timeout > 0) {
                timer = setTimeout(function () {
                    ...
                    xhr.abort();
                    onerror({result: {code: "ER_TIMEOUT", summary: "timeout"}})
                }, timeout)
            }
            xhr.onreadystatechange = function (data) {
                if (xhr.readyState == 4) {
                    ...
                    if (xhr.status != 0) {
                        ...
                        onerror({result: {code: "ER_STATUS", summary: "status is 0"}})
                    }
                }
            };
